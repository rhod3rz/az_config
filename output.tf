output "item_name" {
  value = {
    ...
    sa_primary                 = lower("sa${var.environment}${lookup(var.region_shortname_mapping, var.location_primary)}${var.application_code}${var.unique_time}")
    sa_secondary               = lower("sa${var.environment}${lookup(var.region_shortname_mapping, var.location_secondary)}${var.application_code}${var.unique_time}")
